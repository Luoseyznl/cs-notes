...
用极简的单行代码替代多行的 `for` 循环和 `append` 操作的表达式。
...
生成器（对象）：**带有 `yield` 关键字的函数会返回一个生成器对象**，每次调用 `next()` 会执行到下一个 `yield` 语句并返回值，直到没有 `yield` 时抛出 `StopIteration` 异常。
