...
\tikzset{
    ...
    pcStamp/.pic={
        \begin{scope}[scale=0.6, thick, draw=black!70]
            ...
            \node[font=\bfseries, text=themeBlue, xshift=16pt, yshift=-5pt] at (0, 0) {WORK};
        \end{scope}
    },
    %
    % 印章：小人
...
\begin{tikzpicture}
	...
	\node (live) [C, below right=of man] {\Large \textbf{LIVE}};

	%% 连线 \draw[样式] (起点) -- 途中节点 (终点);
	\draw[conn, <->, rounded corners=15pt] (pc) -| node [above] {Code} (man); % node [above] 在连线间塞入文本节点
	\draw[conn, ->, rounded corners=30pt] (man) |- node [below] {Deploy} (live);
