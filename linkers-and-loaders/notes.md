...
计算机系统是从上到下分层的，而**接口总是由下层指向上层提供的**。例如，硬件层向内核提供“硬件规格(Hardware Specification)”，内核层向运行库提供“系统调用接口(System Call)”，而运行库和操作系统最终向顶层应用提供“API(应用程序编程接口)”。

<div align="center">
  <img src="../images/linkers-and-loaders/computer_system.png" alt="computer system" width="80%">
</div>

[**运行库(Runtime Library)**](#运行库与系统调用) 是支撑程序运行的底层基础设施。在没有操作系统的裸机(Bare Metal)环境下，运行库负责直接初始化内存与硬件栈；而在现代操作系统中，运行库主要负责封装底层的**系统调用接口(System call Interface)**，向上提供标准化的 API。

所有的应用程序都以进程(Process)的方式运行在比操作系统权限更低的级别，每个进程都有自己独立的地址空间，使得进程之间的地址空间相互隔离。

操作系统的两大核心功能：**提供接口**和**管理硬件资源**。
...
### 核心差异：地址无关(PIC) vs 装载时重定位(Rebasing)
...
> 因此，在 C++ 中直接返回大对象会产生多次“深拷贝”(局部对象 $\rightarrow$ 临时对象 $\rightarrow$ 接收变量)。这也是现代 C++ 引入返回值优化(RVO)和移动语义(Move Semantics)的原因。
...
BigObj foo() {
    BigObj local_obj;      // Step 2: 在 foo 栈帧内分配 local_obj 的内存
    local_obj.data[0] = 1; 
    return local_obj;      // Step 3: 将 local_obj 拷贝到 main 的临时对象里，第一次深拷贝
} // 析构，释放掉 local_obj 以及 foo 的栈帧

int main() {
    BigObj result = foo(); // Step 1: 在 main 栈帧内分配临时对象的内存
    // Step 4: foo() 结束后，还要用临时对象拷贝构造 result，完了释放临时对象
    return 0;
}
```

1. 从 `foo` 的栈帧里的 `local_obj` **深拷贝** $\rightarrow$ `main` 栈帧内的**临时对象**。

2. 从 `main` 栈帧里的临时对象**深拷贝** $\rightarrow$ `main` 栈帧中的目标变量 `result`。

因此 C++ 引入了优化机制减少无谓的拷贝：

1. 返回值优化 (RVO - Return Value Optimization)：`foo()` 直接在 `main` 栈帧里分配 `local_obj`，减少一次从局部对象 $\rightarrow$ 临时对象的深拷贝。

2. 移动语义 (Move Semantics)：编译器直接将临时对象内部资源给目标变量(比如堆内存的指针等等)，减少一次从临时对象 $\rightarrow$ 目标变量的深拷贝。
