...
1. 创建虚拟空间（Virtual Address Space）：分配一个空的页表（Page Table），用于记录虚拟空间与物理内存的映射关系。
...
> 3. 操作系统接管，根据 VMA 数据结构从硬盘上的 ELF 文件中读取对应的页，并在页表中记录虚拟空间与物理内存的映射关系；
...
共享库的符号的虚拟地址不确定，地址无关代码（Position-independent Code, PIC）：通过全局偏移表 GOT 确定外部符号地址。全局偏移表存放在数据段 .data 中（因为数据段是每个进程私有的，而代码段却是共享的）。在装载阶段完成 GOT 的解析即可。


## 2 延迟绑定与 PLT 表

相比于静态链接，动态链接在程序装载运行时需要耗费时间进行符号解析，ELF 采用了延迟绑定提高性能：仅当**外部函数**第一次被调用时进行绑定（符号解析），通过代码段中的**过程链接表** PLT（Procedure Linkage Table）实现：

第一次调用时从 PLT 跳转到触发 `_dl_runtime_resolve()` 动态链接器实现符号解析。动态链接器会将正确地址填入 GOT 表。

第二次调用时直接从 PLT 跳转到 GOT 表中的正确地址。

> 为配合延迟绑定，ELF 将 GOT 拆分为 `.got`（存全局变量，装载时立刻绑定）和 `.got.plt`（存外部函数，配合 PLT 延迟绑定）。

## 3 动态链接的 ELF 可执行文件中的特殊段

首先动态链接器的路径由 ELF 可执行文件确定：有一个 `.interp` 段存放了动态链接器路径字符串。

ELF 还有 `.dynamic` 段，其相当于动态链接器所需要的文件头：记录了依赖项、动态符号表位置等。

## 4 运行时加载（Explicit Runtime Linking）

意思是由程序代码自己决定，在**程序运行的过程中**动态加载指定模块，不用时将其卸载。这种机制是实现**插件、驱动等热插拔功能**的底层基础。

动态装载库（Dynamic Loading Library）和共享库（Shared Object, `.so`）在文件格式上**没有区别**。区别在于使用方式：

**普通动态链接（隐式链接）**：在程序启动前，由动态链接器（如 `ld.so`）自动完成装载、符号解析和重定位，对程序员是透明的。

**运行时加载（显式加载）**：在程序启动后，由程序员手动编写代码，主动调用系统 API 来装载和解析。

在 Linux 下主要靠系统动态链接器提供的 4 个核心 API 来实现（包含 `<dlfcn.h>` 头文件）：
* `dlopen`：加载动态库；
* `dlsym`：获取库中指定符号（函数或变量）的虚拟地址；
* `dlclose`：卸载动态库；
* `dlerror`：捕获错误信息；
