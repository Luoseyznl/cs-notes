...
> 因此，在 C++ 中直接返回大对象会产生多次“深拷贝”（局部对象 $\rightarrow$ 隐藏临时对象 $\rightarrow$ 接收变量）。这也是现代 C++ 引入返回值优化（RVO）和移动语义（Move Semantics）的原因。
...
int main() {
    ...
    // Step 2: foo() 结束后，还要用临时对象拷贝构造 result，完了释放临时对象
    return 0;
}
```
